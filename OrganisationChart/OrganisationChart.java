...
public class OrganisationChart {
    ...
    boolean found; // If the node is found in given node subtree
    // Fields
    ...
    private final String newRole = null;
    ...
    private void drawTree(Position pos) {
        ...
        /*# YOUR CODE HERE */
        if (pos.isManager()) { // Checks if the node is not a leaf node
            for (Position member : pos.getTeam()) { // Goes through the team members of the node
                drawTree(member);
            }
        }
    }
    ...
    private Position findPosition(double x, double y, Position pos) {
        ...
        if (pos != null) {
            ...
            if (pos.on(x, y)) { // If the given node is the node at coordinate (X, Y)
                return pos;

            } else {
                List<Position> team = new ArrayList<>(pos.getTeam()); // Creates a list of the team members
                Collections.reverse(team); // Reverse the list of team members

                for (Position member : team) {
                    employ = findPosition(x, y, member);
    ...
    private void movePosition(Position pos, Position target) {
        ...
        /*# YOUR CODE HERE */
        if (!inSubtree(pos, target)) { // Checks if the target node is not in subtree of the pos
            Position oldManager = pos.getManager(); // Gets the manager of the pos
            oldManager.removeFromTeam(pos); // Removes the pos from the old managers team
            target.addToTeam(pos); // Adds the pos to the target manager's team
        }
    ...
    public void removePosition(Position pos) {
        /*# YOUR CODE HERE */
        if (!pos.isManager()) { // Checks if the node is a leaf node
            Position oldManager = pos.getManager(); // Gets the manager of the pos
            oldManager.removeFromTeam(pos); // Removes the pos from the old managers team
        }
    ...
    private boolean inSubtree(Position pos, Position subtree) {
        /*# YOUR CODE HERE */
        found = false;
        if (pos != null && subtree != null) {
            if (pos == subtree) {
                return true;
            } else {
                Set<Position> team = pos.getTeam();
                for (Position member : team) {
                    found = inSubtree(member, subtree);
                    if (found) {
                        return found;
                    }
                }
