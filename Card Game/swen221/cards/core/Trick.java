...
public class Trick {
	...
	public void play(Player p, Card c) throws IllegalMove {
	    ...
	    if (!(p.getDirection() == getNextToPlay())) {
	        throw new IllegalMove("It's not " + p.getDirection() + "'s turn to play.");
	    }

	    // Check if the player has the card in hand
	    if (!isInHand(p, c)) {
	        throw new IllegalMove("Card not in hand: " + c.toString());
	    }

	    // Check if the player has any cards in hand that match the lead suit
	    if (hasCardOfLeadSuit(p, getLeadSuit()) && !isCardInSuit(c, getLeadSuit())) {
	        throw new IllegalMove("Invalid suit: " + c.toString());
	    }
	...
	// Checks if the player has a card of the lead suit
    private boolean hasCardOfLeadSuit(Player p, Card.Suit leadSuit) {
        if (leadSuit != null) {
	...
	// Checks if the card being played is in hand or not
	private boolean isInHand(Player p, Card c) {
		return p.getHand().contains(c);
	}
    
	// Gets the lead suit
    private Card.Suit getLeadSuit() {
        if (cards[0] != null) {
            return cards[0].suit();
        }
        return null;
    ...
    // Checks if the current card suit matches the lead suit
    private boolean isCardInSuit(Card card, Card.Suit suit) {
