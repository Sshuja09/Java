...
public abstract class Tetromino {
   ...
   * */
  public boolean overlap(Board b) { // WRITE THIS USING STREAM
    //note, here we leverage on b.read returning Optional.empty()
    //when x,y are out of the board
	  for (int i = 0; i < 4; i++) {
	        int x = this.x(i);
	        int y = this.y(i);
	        if (!Board.xOk(x) || !Board.yOk(y)) {
	            return true;
	        }
	        Optional<Color> color = b.read(x, y);
	        if (color.isPresent() && color.get() != Color.EMPTY) {
	            return true;
	        }
	    }
	    return false;
  ...
  public void copyOnBoard(Board b) {//TODO: fix this code
    Board.rangeT().forEach(i-> b.write(x(i), y(i), color()));     
  }
}