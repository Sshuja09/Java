...
public class Permutations {
    ...
    public void extendPermutation(Set<String> remainingItems, Stack<String> permutationSoFar, List<List<String>> allPermutations) {
        ...
        if(remainingItems.isEmpty()){
            if(counter <= 10000) {
                List<String> toList = new ArrayList<String>(permutationSoFar);
                allPermutations.add(toList);
            }
            counter++;
        }
        for(String s : remainingItems){
            Set<String> copy = new HashSet<String>(remainingItems); // Copying the remaining items in order to modify it
            copy.remove(s);
            permutationSoFar.push(s);
            extendPermutation(copy, permutationSoFar, allPermutations);
